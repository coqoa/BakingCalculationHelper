...
class _LoginState extends State<Login> {
  ...
  void signupIdValidateFunc(userEmail, signupIdDefault){
    if(userEmail.length < 6){
      setState(() {
        signupIdValidateValue = '6자 이상의 E-Mail을 작성해주세요';
      });
    }
    ...
    return Scaffold(
      ...
      body: Center(
        child: AnimatedContainer(
          ...
          curve: Curves.easeIn,
          width: GetPlatform.isMobile ? MediaQuery.of(context).size.width*0.9 : 300.0,
          // width: MediaQuery.of(context).size.width< 1950 
          //   ? MediaQuery.of(context).size.width*0.6 
          //   : MediaQuery.of(context).size.width*0.5,
          height: GetPlatform.isMobile ? MediaQuery.of(context).size.width*1.3 : 400.5,
          // color: Colors.green,
                    ...
                    onTap: () async{
                      if(!isSignupScreen){
                        ...
                        try{ // 조건 조정하기, 에러메시지 제대로 출력하기 / 한군데서 관리하기(validator통합)
                          // signupIdDefault = true // 서버에서 아이디 체크 
                          // 
                          // signupPasswordValidator = true // 6자 이상 체크
                          // signupPasswordCheckValidator = true // 패스워드 같은지 체크 
